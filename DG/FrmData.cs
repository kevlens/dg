...
namespace DG
{
    public partial class FrmData : Form
    {
        ...
        private void AddPointData(TreeNode sectionNode, int index, string name)
        {
            ...
            var pointList = Constant.client.Queryable<BROKENRAIL_BDATA_DEVICE>().Where(o => o.SECTION_ID == section.SECTION_ID)
                .OrderBy("MILEAGE_COORDINATE").ToList();
        ...
        private void 初始化内网数据ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            List<PushBasicData> pbdList = new List<PushBasicData>();
            var sectionList = Constant.client.Queryable<BROKENRAIL_BDATA_SECTION>().ToList();
            foreach (var item in sectionList)
            {
                pbdList.Add(new PushBasicData
                {
            ...
            var deviceList = Constant.client.Queryable<BROKENRAIL_BDATA_DEVICE>().ToList();
            ...
            var lineList = Constant.client.Queryable<BROKENRAIL_SECTION_LINE>().ToList();
            ...
            var boundaryList = Constant.client.Queryable<BROKENRAIL_BDATA_BOUNDARY>().ToList();
            foreach (var item in boundaryList)
            {
                pbdList.Add(new PushBasicData
                {
                    addtime = DateTime.Now,
        ...
        private void treeView1_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            else
            {
                ...
                var lineInfo = Constant.client.Queryable<BROKENRAIL_SECTION_LINE>().Where(o => o.POINT_NO == device.POINT_NO).First();
                if (lineInfo != null)
                {
        ...
        {
            if (MessageBox.Show("退出将清除所有操作记录！是否退出？", "退出确认", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                this.Dispose();
                Constant.opList = new List<PushBasicData>();
                //    Application.Exit();
            }
            else
            {
        ...
        private void 生成操作SQLToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                Constant.SaveFile(Constant.opList, "");
                Constant.opList = new List<PushBasicData>();
            }
        }
        ...
        private void textBox1_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == '\r')
            {
                ...
                {
                    nextnodes(item, textBox1.Text.Trim());
                }
            }
        }
        ...
        private void 备份ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var sectionPath = Application.StartupPath + "\\区间备份" + Constant.pathext + ".xls";
            var devicePath = Application.StartupPath + "\\设备备份" + Constant.pathext + ".xls";
            var linePath = Application.StartupPath + "\\线路备份" + Constant.pathext + ".xls";
            if (new ExcelHelper<BROKENRAIL_BDATA_SECTION>().ToExcel(sectionPath, "区间", "区间", sectionList, "BROKENRAIL_BDATA_SECTION"))
            {
                MessageBox.Show("区间备份成功");
            }
            else
            {
                MessageBox.Show("区间备份失败");
            }
            if (new ExcelHelper<BROKENRAIL_BDATA_DEVICE>().ToExcel(devicePath, "设备", "设备", deviceList, "BROKENRAIL_BDATA_DEVICE"))
                MessageBox.Show("设备备份成功");
            else
                MessageBox.Show("设备备份失败");
            if (new ExcelHelper<BROKENRAIL_SECTION_LINE>().ToExcel(linePath, "线路", "线路", lineList, "BROKENRAIL_SECTION_LINE"))
                MessageBox.Show("线路备份成功");
            else
                MessageBox.Show("线路备份失败");
        }

        private void 新增线路数据ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            FrmGisLine fgl = new FrmGisLine();
            fgl.ShowDialog();
        }

        private void 导入线路数据ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            List<PushBasicData> pdList = new List<PushBasicData>();
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                List<T_GIS_LINE> gisLines = new ExcelHelper<T_GIS_LINE>().ToDataTable(openFileDialog1.FileName, 0, 1);
                if (gisLines.Count > 0)
                {
                    foreach (var item in gisLines)
                    {
                        PushBasicData pbd = new PushBasicData
                        {
                            addtime = DateTime.Now,
                            ...
                            status = 0,
                            errnum = 0,
                            datatext = Newtonsoft.Json.JsonConvert.SerializeObject(item)
                        };
                        pdList.Add(pbd);
                    }
                    Constant.SaveFile(pdList, "");
                }
                else
                {
                    MessageBox.Show("文件读取错误");
        ...
        private void 新增车站数据ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            FrmGisStation fgs = new FrmGisStation();
            fgs.ShowDialog();
        }

        private void 导入车站数据ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            List<PushBasicData> pdList = new List<PushBasicData>();
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                List<T_GIS_STATION> gisStations = new ExcelHelper<T_GIS_STATION>().ToDataTable(openFileDialog1.FileName, 0, 1);
                if (gisStations != null && gisStations.Count > 0)
                {
                    foreach (var item in gisStations)
                    {
                        PushBasicData pbd = new PushBasicData
                        {
                            addtime = DateTime.Now,
